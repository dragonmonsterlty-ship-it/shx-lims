...
- `id`、`code`（对照品编号，唯一，非空。**默认由系统按 `RS-<年份>-<三位流水>`
  自动生成**，入库表单中可手工覆盖以兼容历史编号体系；唯一校验兜底）、`name`（非空）
...
- `id`、`ref_standard_id`（FK）、`used_by`（FK user，登记即当前登录人）
- `reviewed_by_name`（复核人，`String(100)` 自由文本，**必填**——对齐纸面台账
  "领取人 + 复核人 + 用途"三件套，不要求是系统账号）
- `amount`（`Numeric(18,4)`）、`unit`、`used_at`、`purpose`（用途，**必填**）
- `created_at`
...
- **M2 对照品台账**：数据表、入库/编辑/报废、列表/详情、附件、审计、
  **历史台账导入**——将既有杂质对照品 Excel（约 38 条带量记录）经种子/导入脚本
  灌入 `ref_standard` 表。解析规则已在单机版原型中验证：合并单元格向下填充、
  "0.9g" 拆数量与单位、纯度小数归一为百分比、"2025.7.24" 等日期格式归一，
  可直接翻译为 Python。
  验收标准：合成账号可入库并挂 COA，QC 账号可查询，历史数据完整入库。
- **M3 领用闭环**：流水表、领用登记、余量扣减、效期高亮、demo seed 与文档。
  验收标准：QC 账号完成一次领用，余量与流水正确，审计可查。

## 已确认决议（2026-07-13）

1. 编号默认自动生成 `RS-<年份>-<三位流水>`，入库表单可手工覆盖，唯一校验兜底。
2. 不做"仅合成可入库"的系统硬限制，靠操作约定 + 审计追责；将来 `department`
   规范为枚举后，收紧方式是入库按钮按模块 + 部门显隐，而非接口层拒绝。
3. 不新增角色：QC/QA 用 `operator` + 仅 `refstd` 模块，只读用 `viewer`。
4. 领用人即当前登录人，不支持代登记；流水必填复核人（自由文本）与用途。
